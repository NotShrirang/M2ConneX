...
class Feed(CODEBaseModel):
    subject = models.CharField(verbose_name=_(
        'Subject'), max_length=100, null=False, blank=False)
    # TODO: Add Rich Text Field
    body = models.TextField(verbose_name=_('Body'), null=False, blank=False)
    user = models.ForeignKey(AlumniPortalUser, verbose_name=_(
...
class FeedImage(CODEBaseModel):
    feed = models.ForeignKey(
        Feed, on_delete=models.CASCADE, related_name='images')
...
class FeedAction(CODEBaseModel):
    ...
    feed = models.ForeignKey(
        Feed, on_delete=models.CASCADE, related_name='actions')
    action = models.CharField(
        max_length=10, choices=FEED_ACTIONS, null=False, blank=False)
    user = models.ForeignKey(
        AlumniPortalUser, on_delete=models.CASCADE, related_name='feed_actions')
    ...
    def __str__(self):
        return f"{self.user.firstName} {self.user.lastName} - {self.action} - {self.feed.subject}"


class FeedActionComment(CODEBaseModel):
    feedAction = models.ForeignKey(
        FeedAction, on_delete=models.CASCADE, related_name='comments')
